...
class ProgramSankey(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        if five_query.exists():
            ...
            percentage_one = 0
        program = (
            five_query.values("program_id__name", "program_id", "program_id__code")
            .exclude(allocated_budget__lt=percentage_one)
            .filter(program_id__in=program_filter_id)
            .distinct("program_id")
        )
        ...
        component = (
            five_query.values(
                "component_id__name", "component_id", "component_id__code"
            )
            .exclude(allocated_budget__lt=percentage_one)
            .filter(program_id__in=program_filter_id)
        ...
        partner = (
            five_query.values("supplier_id__name", "supplier_id", "supplier_id__code")
            .exclude(allocated_budget__lt=percentage_one)
            .filter(program_id__in=program_filter_id)
        ...
        for i in range(0, len(component_id)):
            q = (
                ...
                .exclude(allocated_budget__lt=percentage_one)
                .filter(component_id=component_id[i])
            )
        ...
        for i in range(0, len(component_id)):
            q = (
                ...
                .exclude(allocated_budget__lt=percentage_one)
                .filter(supplier_id__in=partner_id, component_id=component_id[i])
            )
...
class RegionSankey(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        if request.GET.getlist("province"):
            ...
            province_filter_id = list(
                Province.objects.exclude(code=-1)
        ...
        if five_query.exists():
            ...
            percentage_one = 0
        province = (
            five_query.values("province_id__name", "province_id", "province_id__code")
        ...
        district = (
            five_query.values(
                "province_id", "district_id__name", "district_id", "district_id__code"
            )
            .distinct("district_id")
            .exclude(allocated_budget__lt=percentage_one)
            .filter(~Q(district_id__code=-1))
        )
        for c in district:
            node.append(
                {
        ...
        municipality = (
            ...
            .exclude(allocated_budget__lt=percentage_one)
            .filter(~Q(municipality_id__code=-1))
        )
        for part in municipality:
            ...
            indexes.append(
                part["municipality_id__name"] + str(part["municipality_id__code"])
            )
        ...
        for i in range(0, len(district_id)):
            q = (
                ...
                .filter(district_id=district_id[i])
                .exclude(allocated_budget__lt=percentage_one)
            )
        ...
        for i in range(0, len(municipality_id)):
            q = (
                ...
                .filter(municipality_id=municipality_id[i])
                .exclude(allocated_budget__lt=percentage_one)
            )
    ...
    def list(self, request, *args, **kwargs):
        ...
        if request.GET.getlist("region") and request.GET.getlist("program_id"):
            if request.GET["region"] == "province":
                ...
                fivew = (
                    FiveW.objects.filter(program_id=programid)
                    .values(
                        "id",
                        "allocated_budget",
                        "province_id",
                ...
                return Response(
                    {
                        ...
                        "municiaplity_count": municipality_count,
                        "federal_level_components": finaldata,
                        "activemap": activemap,
                    }
                )
                ...
                return Response(
                    {
                        ...
                        "end_date": program.end_date,
                        "description": program.description,
                        "total_budget": total_budget["allocated_budget__sum"],
                        "province_count": province_count,
                        "district_count": district_count,
                        "municiaplity_count": municipality_count,
                        "federal_level_components": finaldata,
    ...
    def list(self, request, *args, **kwargs):
        if request.GET["region"] == "province":
            ...
            if "province_code" in request.GET:
                ind = (
                    Indicator.objects.filter(federal_level__in=["province", "all"])
                    .exclude(
                        Q(show_flag=False)
                        | Q(is_dashboard=False)
                        | Q(is_regional_profile=False)
                    )
                    .values(
                        "category", "id", "federal_level", "full_title", "indicator"
                    )
                    .distinct()
                )
                ...
                for d in ind:
                    if d["federal_level"] == "province":
                        initial_sum = 0
                        test = IndicatorValue.objects.filter(
                            indicator_id__id=d["id"],
                            province_id__code=int(request.GET["province_code"]),
                        ).values("value")
                        for test in test:
                        ...
                        ).values("value", "indicator_id__unit")
                ...
                if len(supplieruniqueid) != 0:
                    for l in supplieruniqueid:
                        ...
                        fivenew = FiveW.objects.filter(
                            supplier_id=l,
                            province_id__code=int(request.GET["province_code"]),
                        ).values("supplier_id__name", "supplier_id", "allocated_budget")
                        for f in fivenew:
                ...
                if len(uniqueprogramid) != 0:
                    for p in uniqueprogramid:
                        pr = (
                            ...
                            .exclude(allocated_budget=None)
                            .values(
                                "allocated_budget",
                                "program_id__name",
                                "program_id__id",
                                "program_id__program_acronym",
                            )
                        )
                        top_prog_by_budget.append(
                            {
                                ...
                                "key": "total_budget",
                                "value": pr.aggregate(Sum("allocated_budget"))[
                                    "allocated_budget__sum"
                                ],
                            }
                        )
                test2 = sorted(
                    top_prog_by_budget, key=lambda i: i["value"], reverse=True
                )
                test3 = sorted(
                    top_part_by_budget, key=lambda i: i["value"], reverse=True
                )
                total_budget = five.aggregate(Sum("allocated_budget"))[
                    "allocated_budget__sum"
                ]
                sector_count = (
                    five.distinct("program_id__sector")
                    .exclude(program_id__sector=None)
                    .count()
                )
            ...
            if "district_code" in request.GET:
                ind = (
                    Indicator.objects.filter(federal_level__in=["district", "all"])
                    .exclude(
                        Q(show_flag=False)
                        | Q(is_dashboard=False)
                        | Q(is_regional_profile=False)
                    ...
                    .values(
                        "category", "id", "federal_level", "indicator", "full_title"
                    )
                    .distinct()
                )
                for d in ind:
                    if d["federal_level"] == "district":
                        ...
                        test = IndicatorValue.objects.filter(
                            indicator_id__id=d["id"],
                            district_id__code=int(request.GET["district_code"]),
                        ).values("value")
                        for test in test:
                            initial_sum += test["value"]
                        data.append(
            ...
            if "municipality_code" in request.GET:
                ind = (
                    Indicator.objects.filter(federal_level__in=["palika", "all"])
                    .exclude(
                        Q(show_flag=False)
                        | Q(is_dashboard=False)
                        | Q(is_regional_profile=False)
                ...
                for d in ind:
                    ...
                    test = IndicatorValue.objects.filter(
                        indicator_id__id=d["id"],
                        gapanapa_id__code=int(request.GET["municipality_code"]),
                ...
                five = (
                    FiveW.objects.filter(
                        municipality_id__code=int(request.GET["municipality_code"])
                    )
                    .exclude(
                        municipality_id__code="-1",
                        district_id__code="-1",
                ...
                if len(uniqueprogramid) != 0:
                    for p in uniqueprogramid:
                        pr = (
                            FiveW.objects.filter(
                                program_id=p,
                                municipality_id__code=int(
                                    request.GET["municipality_code"]
                                ),
                            )
                            .exclude(allocated_budget=None)
                            .values(
...
class ProgramUpperDendrogram(viewsets.ReadOnlyModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        if request.GET.getlist("program_id"):
            ...
            programid = request.GET["program_id"]
            dami = (
                Project.objects.filter(program_id=programid).values("name").distinct()
            )
            for d in dami:
                partner = []
                if d["name"] not in component:
                    component.append({"name": d["name"], "children": partner})
                nadami = (
                    Project.objects.filter(name=d["name"])
                    .values("partner_id__name")
                    .distinct()
                )
                for n in nadami:
                    if n["partner_id__name"] not in partner:
                        partner.append({"name": n["partner_id__name"]})
    ...
    def list(self, request, *args, **kwargs):
        if request.GET.getlist("program_id"):
            ...
            for d in dami:
                province = []
                if d["name"] not in component:
                    component.append({"name": d["name"], "children": province})
                nadami = (
                    FiveW.objects.filter(component_id__name=d["name"])
                    .exclude(
                        municipality_id__code="-1",
                        district_id__code="-1",
                        province_id__code="-1",
                    )
                    .values("province_id__name", "province_id__code")
                    .distinct()
                )
                for n in nadami:
                    district = []
                    ...
                    dist = (
                        FiveW.objects.filter(
                            province_id__name=n["province_id__name"],
                            component_id__name=d["name"],
                        )
                        .exclude(
                            municipality_id__code="-1",
                            district_id__code="-1",
                        ...
                        .values("district_id__name", "district_id__code")
                        .distinct()
                    )
    ...
    def list(self, request, *args, **kwargs):
        ...
        if request.GET["region"] == "province":
            if request.GET.getlist("province_code"):
                fiveprogram = (
                    FiveW.objects.filter(province_id__code=request.GET["province_code"])
                    .exclude(
                        municipality_id__code="-1",
                        district_id__code="-1",
                        province_id__code="-1",
                    )
                    .values("program_id__name")
                    .distinct()
                )
                for f in fiveprogram:
                    ...
                    dami = (
                        FiveW.objects.filter(
                            program_id__name=f["program_id__name"],
                            province_id__code=request.GET["province_code"],
                        )
                        .values("component_id__name")
                        .distinct()
                    )
                    program.append(
                        {"name": f["program_id__name"], "children": component}
                    )
                    for d in dami:
                        partner = []
                        if d["component_id__name"] not in component:
                            component.append(
                                {"name": d["component_id__name"], "children": partner}
                            )
                        nadami = (
                            FiveW.objects.filter(
                                component_id__name=d["component_id__name"],
                                province_id__code=request.GET["province_code"],
                            )
                            .values("supplier_id__name")
                            .distinct()
                        )
                        for n in nadami:
                            if n["supplier_id__name"] not in partner:
                                partner.append({"name": n["supplier_id__name"]})
            else:
                return Response({"results": "Please Pass Province Code"})
            ...
            if request.GET.getlist("district_code"):
                fiveprogram = (
                    FiveW.objects.filter(district_id__code=request.GET["district_code"])
                    .exclude(
                        municipality_id__code="-1",
                        district_id__code="-1",
                        province_id__code="-1",
                ...
                for f in fiveprogram:
                    ...
                    dami = (
                        FiveW.objects.filter(
                            program_id__name=f["program_id__name"],
                            district_id__code=request.GET["district_code"],
                        )
                        .values("component_id__name")
                        .distinct()
                    )
            ...
            if request.GET.getlist("municipality_code"):
                fiveprogram = (
                    FiveW.objects.filter(
                        municipality_id__code=request.GET["municipality_code"]
                    )
                    .exclude(
                        municipality_id__code="-1",
                        district_id__code="-1",
...
class DistrictIndicator(viewsets.ModelViewSet):
    ...
    def list(self, request, **kwargs):
        ...
        *required= id of indicator as param{indicator_id} send as get request - /district-indicator/?indicator_id={indicator_id}
        ...
        district = (
            District.objects.filter(province_id__id__in=province_ids)
            .values("name", "id", "n_code", "code")
            .exclude(code=-1)
            .order_by("id")
        )
        ...
        for i in range(0, len(id_indicator)):
            ...
            if cat_in.federal_level == "district":
                indicator_dist = IndicatorValue.objects.values(
                    "id", "indicator_id", "value", "district_id__code"
                ).filter(
                    indicator_id=id_indicator[i],
                    dsitrict_id__province_id__id__in=province_ids,
                )
                ...
                for dist in district:
                    ...
                    indicator = IndicatorValue.objects.values(
                        "id", "indicator_id", "value", "indicator_id__unit"
                    ).filter(
                        indicator_id=int(id_indicator[i]),
...
class ProvinceIndicator(viewsets.ModelViewSet):
    ...
    def list(self, request, **kwargs):
        ...
        *required= id of indicator as param{indicator_id} send as get request - /province-indicator/?indicator_id={indicator_id}
        ...
        province = (
            Province.objects.values("name", "id", "code")
            .exclude(code=-1)
            .order_by("id")
        )
        id_indicators = request.GET["indicator_id"]
...
class MarkerValueApi(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        queryset = MarkerValues.objects.select_related("marker_category_id").order_by(
            "id"
        )
        return queryset
...
class FiveWDistrict(viewsets.ReadOnlyModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        if request.GET.getlist("province_code"):
            ...
            districts = (
                District.objects.values(
                    "name", "id", "code", "n_code", "province_id__name"
                )
                .filter(province_id__code__in=province_codes)
                .exclude(code="-1")
                .order_by("id")
            )
        ...
        for dist in districts:
            if count:
                if len(count) == 8:
                    query = FiveW.objects.filter(district_id=dist["id"]).values(
                        "allocated_budget", "component_id", "program_id"
                    )
                else:
                    query = fivew_district(
                        [dist["id"]],
                        supplier,
                        program,
                        component,
            ...
            if request.GET.getlist("field"):
                ...
                kwargs = {"{0}__iexact".format(field): value}
            ...
            if query:
                ...
                if request.GET.getlist("sector_id"):
                    if not request.GET.getlist("sub_sector_id"):
                        da = query.values(
                            "program_id__id",
                            "program_id__sector_budget",
                            "program_id__total_budget",
                            "allocated_budget",
                        )
                        sub_sec = [
                            i.id for i in SubSector.objects.filter(sector_id__in=sector)
                        ]
                        for d in da:
                            sec_budget = 0
                        ...
                        for d in da:
                            ...
                            if d["allocated_budget"]:
                                total_new_budget1 += (
                                    d["allocated_budget"] * sub_sec_budget
                                ) / 100
                ...
                sect = (
                    query.exclude(program_id__sector__name=None)
                    .values_list("program_id__sector__name", flat=True)
                    .distinct("program_id__sector__name")
                )
                sub_sect = (
                    query.exclude(program_id__sub_sector__name=None)
                    .values_list("program_id__sub_sector__name", flat=True)
                    .distinct()
                )
                for s in sub_sect:
                    print(s)
                mark = (
                    query.exclude(program_id__marker_category__name=None)
                    .values_list("program_id__marker_category__name", flat=True)
                    .distinct()
                )
                mark_val = (
                    query.exclude(program_id__marker_value__value=None)
                    .values_list("program_id__marker_value__value", flat=True)
                    .distinct()
                )
...
class FiveWProvince(viewsets.ReadOnlyModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        for province in provinces:
            if count:
                if len(count) == 8:
                    query = FiveW.objects.filter(province_id=province["id"]).values(
                        "allocated_budget", "component_id", "program_id"
                    )
                else:
                    query = fivew_province(
                        [province["id"]],
                        supplier,
                        program,
                        component,
    ...
    def list(self, request, *args, **kwargs):
        ...
        if request.GET.getlist("province_code"):
            ...
            for i in range(0, len(province_codes)):
                province_codes[i] = int(province_codes[i])
            municipalities = (
                GapaNapa.objects.values(
                    "name", "id", "code", "province_id__name", "district_id__name"
                )
                .filter(province_id__code__in=province_codes)
                .order_by("id")
            )
        ...
        if request.GET.getlist("district_code"):
            ...
            municipalities = (
                ...
                .filter(district_id__code__in=district_codes)
                .exclude(code="-1")
                .order_by("id")
            )
        ...
        for municipality in municipalities:
            if count:
                if len(count) == 8:
                    ...
                    ).values("allocated_budget", "component_id", "program_id")
                else:
                    query = fivew_municipality(
                        [municipality["id"]],
                        supplier,
                        program,
                        component,
            ...
            if query.exists():
                ...
                mark_value = query.values_list(
                    "program_id__marker_value__value", flat=True
                ).distinct()
...
class SummaryData(viewsets.ReadOnlyModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        if request.GET.getlist("sector_id"):
            ...
            if (
                query.aggregate(Sum("allocated_budget")).get("allocated_budget__sum")
                is None
            ):
...
class IndicatorApi(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        queryset = Indicator.objects.exclude(
            show_flag=False, is_dashboard=False
        ).order_by("id")
        return queryset
    ...
    def get_queryset(self):
        ...
        if self.request.GET.getlist("province_id"):
            ...
            queryset = (
                IndicatorValue.objects.filter(
                    indicator_id__in=id_indicator,
                    gapanapa_id__district_id__province_id__id__in=province_ids,
                )
                .select_related("gapanapa_id", "indicator_id")
                .order_by("id")
            )
        ...
        if self.request.GET.getlist("district_id"):
            ...
            queryset = (
                IndicatorValue.objects.filter(
                    indicator_id__in=id_indicator,
                    gapanapa_id__district_id__id__in=district_ids,
                )
                .select_related("gapanapa_id", "indicator_id")
                .order_by("id")
            )
            ...
            if self.request.GET.getlist("province_id") == []:
                print("herer")
                queryset = (
                    IndicatorValue.objects.filter(indicator_id__in=id_indicator)
                    .select_related("gapanapa_id", "indicator_id")
                    .order_by("id")
                )
...
class ProgramTestApi(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        elif self.request.GET.getlist("start_date") and self.request.GET.getlist(
            "end_date"
        ):
...
class CovidChoice(viewsets.ReadOnlyModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        return Response(
            {
                "field": [
                    {"name": "Kathmandu Activity", "value": "kathmandu_activity"},
                    {
    ...
    def list(self, request, *args, **kwargs):
        ...
        if query.exists():
            ...
            for p in program:
                ...
                if request.GET.getlist("sector_id") or request.GET.getlist(
                    "sub_sector_id"
                ):
                    sec_id_list = [i["id"] for i in pid.sector.all().values("id")]
                    pt = len(list(set(sec_id_list).intersection(sector)))
                    sub_sec_id_list = [
                        i["id"] for i in pid.sub_sector.all().values("id")
                    ]
                ...
                c_data = (
                    query.values("component_id", "component_id__name")
                    .filter(program_id=p["program_id"])
                    .annotate(Sum("allocated_budget"))
                )
                ...
                for c in c_data:
                    ...
                    part_data = (
                        query.values("supplier_id", "supplier_id__name")
                        .filter(
                            program_id=p["program_id"], component_id=c["component_id"]
                        )
                        .annotate(Sum("allocated_budget"))
                    )
                ...
                if request.GET.getlist("sector_id"):
                    program_budget = (
                        (p["allocated_budget__sum"] * sec_budget) / 100
                    ) / pt
                elif request.GET.getlist("sub_sector_id"):
                    program_budget = (
                        (p["allocated_budget__sum"] * sec_budget) / 100
        ...
        if request.GET["region"] == "province":
            ...
            if "province_code" in request.GET:
                ...
                if len(sectoruniqueid) != 0:
                    for s in sectoruniqueid:
                        ...
                        dat = FiveW.objects.filter(
                            program_id__sector=s,
                            province_id__code=int(request.GET["province_code"]),
                        ).exclude(allocated_budget=None)
                        sector = Sector.objects.get(id=s)
                        set = SubSector.objects.filter(sector_id=sector.id).values(
                            "name"
                        )
                        for v in set:
                        ...
                        sub_sec = [
                            i["id"]
                            for i in SubSector.objects.filter(
                                sector_id=sector.id
                            ).values("id")
                        ]
                        total_budgetnew = 0
                        ...
                        for d in dat:
                            ...
                            sub_sectors.append(
                                list(
                                    d.program_id.sub_sector.values_list(
                                        "name", flat=True
                                    )
                                )
                            )
                        ...
                        partner_count_temp = (
                            FiveW.objects.filter(
                                supplier_id__name__in=partner_name,
                                province_id__code=request.GET["province_code"],
                            )
                            .values("supplier_id")
                            .distinct("supplier_id")
                ...
                test4 = sorted(
                    top_sector_by_no_partner, key=lambda i: i["value"], reverse=True
                )
                return Response(
                    {"active_sectors": test1, "top_sector_by_no_of_partner": test4}
                )
            else:
                return Response({"results": "Please Pass Province Code"})
            ...
            if "municipality_code" in request.GET:
                five = (
                    ...
                    .values("id", "program_id__sector__id")
                    .distinct()
                )
